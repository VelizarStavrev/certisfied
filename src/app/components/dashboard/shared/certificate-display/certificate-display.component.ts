...
export class CertificateDisplayComponent implements OnInit {
  ...
  @Output() updateFieldFunc = new EventEmitter<{ fieldId: number, fieldX: string, fieldY: string }>();
  ...
  updateFieldPosition(event: MouseEvent, fieldId: number) {
    ...
    const pageHeaderHeight = variables.headerHeight; // Due to the position of the main component, the offsetTop excludes the header
